# Build it with:
#    nix-build gerbil-docker.nix
# Load it with:
#    docker load < ./result |& tee foo &&
#    a=$(tail -1 foo | cut -d' ' -f3) &&
#    docker tag $a fahree/gerbil-nix &&
#    docker run -t -i $a -c zsh
