...
(import
  ...
  :std/test
  ../base ../number ../extensible-vector)

(def extensible-vector-test
  (test-suite "test suite for clan/extensible-vector"
    (test-case "evector"
      (def e (evector<-list '(a b c d)))
      ...
      (check-equal? (vector<-evector e) #(a b c d x)))
    (test-case "ebytes"
      (def e (ebytes<-string "abcd"))
      ...
      (check-equal? (bytes<-ebytes e) #u8(97 98 99 100 120)))
    (test-case "ebits"
      (def e (ebits<-bits #xAC 11))
      ...
      (check-equal? (values->list (bits<-ebits e)) '(665772 22)))
    ))
