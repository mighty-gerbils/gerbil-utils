...
(defsyntax (.defgeneric stx) ;; define a generic function that invokes the prototype's slot
  ...
  (def (parse-options options)
    ...
    (let loop ((options options))
      (match options
        ([] (values slot-name default from-type))
        ([kw val . rest]
         (match (syntax-e kw)
           (slot: (set! slot-name val))
           (from: (case (syntax-e val)
                    ((type) (set! from-type #t))
                    ((instance) (set! from-type #f))
  ...
  (syntax-case stx ()
    ((.defgeneric formals options ...)
     (begin
       ...
       (define-values (slot-name default use-type) (parse-options (syntax->list #'(options ...))))
       (with-syntax* ((gf fun)
                      (self self)
                      ((args ...) (or funargs #'()))
                      ...
                      (methods (if use-type #'(.get self .type methods) #'self))
                      (getter #'(.ref methods 'slot-name base))
                      ((evars ...) (if use-type #'(self vars ...) #'(vars ...))))
         (if (or funargs use-type)
