...
(def (open-transaction (c (current-db-connection)))
  ...
  transaction)
(def (call-with-new-tx fun (c (current-db-connection)))
  (let (tx (open-transaction c))
    (parameterize ((current-db-transaction tx))
      (fun tx))
    (commit-transaction tx)))
(def (call-with-tx fun (tx (current-db-transaction)))
  (if tx (fun tx) (call-with-new-tx fun)))
...
(def (db-get key (c (current-db-connection)) (opts (leveldb-default-read-options)))
  (leveldb-get (DbConnection-leveldb c) key opts))
(def (db-key? key (c (current-db-connection)) (opts (leveldb-default-read-options)))
  (leveldb-key? (DbConnection-leveldb c) key opts))
