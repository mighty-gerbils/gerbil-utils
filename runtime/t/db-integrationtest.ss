...
(import
  ...
  :clan/utils/concurrency :clan/utils/number :clan/utils/path-config
  ../db)

(def db-integrationtest
  (test-suite "test suite for glow/runtime/db"
    ...
    (test-case "test-trivial-testdb-put-get"
      ...
      (with-db-connection (c (run-path "testdb"))
        (with-tx (tx) (db-put! key (bytes<-nat test-val) tx))
        (check-equal? (nat<-bytes (db-get key)) test-val)
        (with-tx (tx) (db-put! key (bytes<-nat (1+ test-val)) tx))
        (check-equal? (nat<-bytes (db-get key)) (1+ test-val))))
    (test-case "test-trivial-testdb-commits"
      (def test-base (* 65536 (random-integer 65536)))
      (def max-test-int 200)
      (def failures [])
      (def (test-commit i)
        (def key (bytes<-nat (+ i 65536)))
        (def val (string->bytes (number->string (+ test-base i))))
        (try
         ;; There is a bug in Gambit v4.9.3-1169-gdac90f09 that causes
         ;; the current-db-transaction to sometimes leak to other threads.
      ...
      (with-db-connection (c (run-path "testdb"))
        ...
        (parallel-map test-commit (iota max-test-int 1))
      ...
      (check (- final-batch-id initial-batch-id) ? (cut <= 1 <> 10))
      (check failures ? null?))))
